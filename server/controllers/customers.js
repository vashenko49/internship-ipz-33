...
exports.createCustomer = async (req, res) => {
    try {
        ...
        const userAvatar =
            values.length > 0
                ? await cloudinary.uploader.upload(values[0].path, {folder: `${process.env.cloudinary_path}/userAvatar`})
                : null;
    ...
    try {
        ...
        if (Customer) {
            ...
            const userImg = await cloudinary.uploader.upload(customer.avatarUrl, {
                folder: `${process.env.cloudinary_path}/userAvatar`
            });
